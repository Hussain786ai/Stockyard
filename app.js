/* ═══════════════════════════════════════════
   STOCKYARD — app.js  v2
   Units: Nos / Kgs / Box (selectable per adjustment)
═══════════════════════════════════════════ */
...
// ─── STATE ───────────────────────────────────────────────
let db = null;
let items = [];       // { id, name, stock, min, unit, updatedAt }
let activity = [];
...
function initDB() {
  return new Promise((resolve, reject) => {
    const req = indexedDB.open('stockyard_db', 2);
...
async function init() {
  ...
  for (const item of items) {
    if (!item.unit) { item.unit = 'Nos'; await dbPut(item); }
  }
...
function setActiveUnitBtn(unit) {
  qsAll('#unitSelector .unit-btn').forEach(btn => {
...
async function adjustStock(delta) {
  ...
  const unit = item.unit || 'Nos';
  logActivity(icon, `${label} ${formatStock(qty, activeUnit)} ${activeUnit} × ${item.name} → ${formatStock(newStock, unit)} ${unit}`);
...
async function addItem() {
  ...
  const item = {
    ...
    name, stock, min, unit: unitVal, updatedAt: Date.now()
  };
  await dbPut(item);
  items.push(item);
  logActivity('⊕', `Added ${name} (${formatStock(stock, unitVal)} ${unitVal})`);
...
function exportCSV() {
  ...
  const rows = [['Name', 'Stock', 'Unit', 'Min Alert', 'Status']];
  items.forEach(i => {
    const unit = i.unit || 'Nos';
    rows.push([`"${i.name.replace(/"/g, '""')}"`, formatStock(i.stock, unit), unit, formatStock(i.min, unit), getStatus(i)]);
  });
...
function importCSV(file) {
  ...
  reader.onload = async e => {
    ...
    for (const row of rows) {
      ...
      let stock, unit, min;
      if (cols.length >= 4 && ['Nos','Kgs','Box'].includes(cols[2])) {
        stock = parseFloat(cols[1]) || 0; unit = cols[2]; min = parseFloat(cols[3]) || 0;
      } else {
      ...
      const item = {
        id: crypto.randomUUID ? crypto.randomUUID() : Date.now().toString(36) + Math.random().toString(36).slice(2),
        name, stock, unit, min, updatedAt: Date.now()
      };
      await dbPut(item); items.push(item); added++;
    }
...
async function editMinStock() {
  ...
  if (!item) return;
  const unit = item.unit || 'Nos';
  const val = prompt(`Set low stock alert threshold for "${item.name}" (${unit}):`, formatStock(item.min, unit));
  if (val === null) return;
  ...
  updateModalStock(item.stock);
  render();
  showToast(`Alert threshold: ${formatStock(num, unit)} ${unit}`);
}
...
async function toggleTheme() {
  ...
  if (isLight) { document.documentElement.removeAttribute('data-theme'); await dbPutMeta('theme', 'dark'); }
  else { document.documentElement.setAttribute('data-theme', 'light'); await dbPutMeta('theme', 'light'); }
}
...
function setupEvents() {
  ...
  qsAll('.unit-btn-form').forEach(btn => {
    btn.addEventListener('click', () => {
      qsAll('.unit-btn-form').forEach(b => b.classList.remove('active'));
      btn.classList.add('active');
      qs('#newUnit').value = btn.dataset.unit;
    });
  });
  ...
  qs('#clearSearch').addEventListener('click', () => {
    qs('#searchInput').value = ''; handleSearch(''); qs('#searchInput').focus();
  });
  ...
  qs('#qtyDown').addEventListener('click', () => {
    const input = qs('#qtyInput');
    const step = stepValue(activeUnit);
  ...
  qs('#importFile').addEventListener('change', e => { importCSV(e.target.files[0]); e.target.value = ''; });
